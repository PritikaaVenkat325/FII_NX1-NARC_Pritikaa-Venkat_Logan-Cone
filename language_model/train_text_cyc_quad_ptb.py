...
def main(args):
  ...
  logger.info("Cycles: ", str(args.cycle))
  logger.info("Warmup: ", str(args.warmup))
  ...
  #Scheduling Vars
  nOc = int((13*args.warmup)+((13*args.warmup)/2))
  a = (-1/(((13*args.warmup)/2)**2))
  count = 32.5 
  logger.info("NOC: ",str(nOc), " A VALUE: ", str(a))
  ...
  while epoch < args.num_epochs:
    ...
    for i in np.random.permutation(len(train_data)):
      if args.warmup > 0:
        ...
        if((args.beta == 1.0) and b > 100 and b < 1359):
          args.beta = 1.0
          #elif b > 400 and epoch >= 39:
          #args.beta = min(1, args.beta + 1.0) 
        elif (count % nOc == 0): 
          count = 32.5
          args.beta = 0.01
          count += 6.5/1359.0
        else:
            args.beta = (a*((count-(13*args.warmup))**2)+1)
            count += 6.5/1359.0
